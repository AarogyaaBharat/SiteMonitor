...
@section('content')
    ...
    <div id="uploadPopup" class="fixed inset-0 bg-[rgba(0,0,0,0.6)] flex items-center justify-center z-50 fade-in"
        ...
        <div class="bg-[var(--bg)] text-[var(--text)] p-6 rounded-2xl w-full max-w-md shadow-xl relative">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
                <!-- PC Upload -->
                <div class="border border-[var(--primary)] rounded-lg p-4 text-center cursor-pointer hover:bg-[var(--primary-light)] transition"
                    onclick="triggerFile('pc')">
                    <svg class="w-8 h-8 mx-auto mb-4 text-[var(--primary)]" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                        </path>
                    </svg>
                    <span class="text-sm">From Computer</span>
                </div>

                <!-- Mobile Upload (will show camera/gallery options on mobile) -->
                {{-- <div class="border border-[var(--primary)] rounded-lg p-4 text-center cursor-pointer hover:bg-[var(--primary-light)] transition"
                    onclick="triggerFile('mobile')">
                    <svg class="w-8 h-8 mx-auto mb-2 text-[var(--primary)]" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                    </svg>
                    <span class="text-sm">From Mobile</span>
                </div> --}}
            </div>

            <!-- Selected File Display -->
            <div id="fileSelectionDisplay" class="hidden">
                <div class="border-2 border-dashed border-[var(--primary)] rounded-xl p-4 text-center">
                    <p id="fileName" class="text-sm font-medium text-[var(--accent)]"></p>
                    <p id="fileSize" class="text-xs text-[var(--text)] opacity-70 mt-1"></p>
                </div>
            </div>

            <!-- Regex Pattern Input -->
            <div class="mt-4">
                <label for="regexPattern" class="block text-sm font-medium mb-1">Regex Pattern (Optional)</label>
                <input type="text" id="regexPattern" name="regex_pattern"
                    class="w-full p-2 border border-[var(--primary)] rounded-lg bg-[var(--bg)] text-[var(--text)]"
                    placeholder="Enter regex pattern to match URLs">
            </div>

            <!-- Action Buttons -->
            <div class="flex justify-between mt-6">
                <button onclick="closePopup()"
                    class="px-4 py-2 rounded-lg border border-[var(--primary)] text-[var(--primary)] hover:bg-[var(--primary-light)] transition">
                    Cancel
                </button>
                <button id="submitBtn" onclick="uploadFile()" disabled
                    class="px-4 py-2 rounded-lg bg-[var(--primary)] text-white hover:bg-[var(--primary-dark)] transition opacity-50 cursor-not-allowed">
                    Check URLs
                </button>
            </div>

            <!-- Hidden File Input -->
            <input id="fileInput" type="file" accept=".csv,.xls,.xlsx,image/*" class="hidden" />
        ...
        <section class="mb-16">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 transition-all">
                ...
                <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                    ...
                    <input onkeyup="onkeyupevent(event);" type="text" id="regex-pattern"
                    ...
                    <button id="upload-trigger-btn" onclick="showUploadPopup()"
                        class="btn bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)] text-white px-6 py-3 rounded-lg font-medium shadow-md hover:shadow-lg">
                        Upload CSV/Excel
                    </button>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            analyzeBtn.addEventListener('click', function() {
                ...
                if (websiteUrl && regexPattern) {
                    ...
                    fetch('/check-urls-regx-result?url=' + encodeURIComponent(websiteUrl) + '&regex=' +
                            encodeURIComponent(regexPattern) + '&depth=5', {
                                method: 'GET',
                                headers: {
                            ...
                            if (data.status === 'success') {
                                ...
                                setTimeout(() => {
                                    document.getElementById('loader-container').classList.add(
                                        'hidden');
                                    const matchesArray = data.results.matches ? Object.keys(data
                                        .results.matches).map(key => ({
                                        url: key,
                                        match_count: data.results.matches[key]
                                            .match_count,
                                        sample_matches: data.results.matches[key]
                                            .matches
                                    })) : [];
    ...
    <script>
        ...
        // Trigger file selection based on device
        function triggerFile(source) {
            const fileInput = document.getElementById('fileInput');
            const cameraInput = document.getElementById('cameraInput');

            if (source === 'mobile' && isMobile()) {
                // On mobile, give option to choose between camera and gallery
                if (confirm("Take photo or choose from gallery?")) {
                    cameraInput.click(); // This will open camera
                } else {
                    fileInput.click(); // This will open gallery
                }
            } else {
                // On PC or if mobile user selected "From Computer"
                fileInput.click();
            }

            // Set up event listeners if not already set
            if (!fileInput.hasAttribute('data-listener')) {
            ...
            if (!cameraInput.hasAttribute('data-listener')) {
        ...
        function handleFileSelection(file) {
            ...
            // Validate file type
            const validExtensions = ['.csv', '.xls', '.xlsx', '.jpg', '.jpeg', '.png'];
            const fileExt = file.name.substring(file.name.lastIndexOf('.')).toLowerCase();

            if (!validExtensions.includes(fileExt)) {
                alert('Please select a valid file (CSV, Excel, or image)');
                return;
            }
            ...
            document.getElementById('fileSelectionDisplay').classList.remove('hidden');
        ...
        function resetFileSelection() {
            ...
            document.getElementById('fileSelectionDisplay').classList.add('hidden');
        ...
        // Upload file function (same as before)
        function uploadFile() {
            const fileInput = document.getElementById('fileInput');
            const cameraInput = document.getElementById('cameraInput');
           ...
           initiateCustomTimer();
            ...
            if (regexPattern) {
                formData.append('pattern', regexPattern);
            }
           
            // Add CSRF token if using Laravel
            formData.append('_token', '{{ csrf_token() }}');
            fetch('/upload-csv-regex', {
                    method: 'POST',
                    body: formData,
                })
                .then(response => response.json())
                .then(data => {
                    ...
                    let matchesHtml = '<li>No matches found.</li>';
                    // If you want to display summary stats from the response, you can do so here:
                    // Example: show total matches or pages with matches
                    // You can also use data.pattern and data.website_url if present
                    if (matchesArray.length > 0) {
                        matchesHtml = matchesArray.map(match =>
                            `<li class="mb-3">
                                <a target='_blank' href='${match.url}'>
                                <div class="flex items-baseline">
                                    <span class="font-medium break-all">${match.url}</span>
                                    <span class="text-sm text-gray-500 dark:text-gray-400 ml-2">
                                        (${match.match_count} ${match.match_count === 1 ? 'match' : 'matches'})
            ...
            // AJAX upload code would go here (same as previous example)
