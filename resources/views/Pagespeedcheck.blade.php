            ...
            <div id="results-section" class="hidden p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1" id="fcp-desc">Time to first content render</div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                        <div class="flex justify-between items-center mb-2">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1" id="tti-desc">Time until page is fully interactive</div>
                    </div>
                </div>
            ...
            <div id="error-section" class="hidden p-6">
                <div class="flex flex-col items-center justify-center py-8 text-center">
                    <div class="h-16 w-16 rounded-full bg-red-100 dark:bg-red-900 flex items-center justify-center mb-4">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-red-500 dark:text-red-400" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <button id="retry-btn" class="mt-4 px-4 py-2 bg-primary-light dark:bg-primary-dark text-white rounded-md hover:bg-opacity-90 transition duration-150 ease-in-out">
                        Try Again
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function showLoading(url) {
            ...
            const interval = setInterval(() => {
                ...
                if (progress < 30) {
                    loadingText.textContent = `Preparing analysis for ${url}`;
                } else if (progress < 60) {
                    loadingText.textContent = `Analyzing page structure for ${url}`;
                } else {
                    loadingText.textContent = `Finalizing results for ${url}`;
                }
            }, 500);
        }
        ...
        function displayResults(data) {
            ...
            const mockData = {
                lighthouseResult: {
                    categories: {
                        performance: { score: data.data.score	, title: 'Performance' },
                        accessibility: { score: data.data.accessibility_score	, title: 'Accessibility' },
                        seo: { score:data.data.seo_score, title: 'SEO' }
                    },
                    audits: {
                        'first-contentful-paint': { displayValue: (data.data.fcp/1000 )+' s', score: 0.9 },
                        'interactive': { displayValue: (data.data.lcp/1000 )+' s', score: 0.85 }
                    }
                }
            };
            
            // Update scores
            document.getElementById('performance-score').textContent = Math.round(mockData.lighthouseResult.categories.performance.score);
            document.getElementById('accessibility-score').textContent = Math.round(mockData.lighthouseResult.categories.accessibility.score);
            document.getElementById('seo-score').textContent = Math.round(mockData.lighthouseResult.categories.seo.score);
            
            // Update status text
            document.getElementById('performance-status').textContent = getPerformanceText(mockData.lighthouseResult.categories.performance.score);
            document.getElementById('accessibility-status').textContent = getPerformanceText(mockData.lighthouseResult.categories.accessibility.score);
            document.getElementById('seo-status').textContent = getPerformanceText(mockData.lighthouseResult.categories.seo.score);
            
            // Update metrics
            document.getElementById('fcp-metric').textContent = mockData.lighthouseResult.audits['first-contentful-paint'].displayValue;
            document.getElementById('tti-metric').textContent = mockData.lighthouseResult.audits['interactive'].displayValue;
            
            // Update progress bars
            document.getElementById('fcp-bar').style.width = `${mockData.lighthouseResult.audits['first-contentful-paint'].score * 100}%`;
            document.getElementById('tti-bar').style.width = `${mockData.lighthouseResult.audits['interactive'].score * 100}%`;
        ...
        function renderMetricsChart(data) {
            ...
            window.metricsChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Scores',
                        data: [data.score, data.accessibility_score, data.seo_score, data.best_practices_score],
                        backgroundColor: [
                            'rgba(59, 130, 246, 0.7)',
                            'rgba(16, 185, 129, 0.7)',
                            'rgba(139, 92, 246, 0.7)',
